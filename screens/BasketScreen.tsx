    ...
    <SafeAreaView className='flex-1 bg-white'>
      <View className='flex-1 bg-gray-100'>
        ...
        <View className='flex-row items-center space-x-4 px-4 py-3 bg-white my-5'>
            ...
            <TouchableOpacity className='text-[#00CCBB]'><Text>Change</Text></TouchableOpacity>
        </View>
        ...
        <ScrollView className='divide-y divide-gray-200'>
            {Object.entries(groupedItemsInBasket).map(([key, items]) => {
                ...
                <View key={key} className='flex-row items-center space-x-3 bg-white py-2 px-5'>
                    ...
                    <TouchableOpacity>
                        <Text className='text-[#00CCBB] text-xs' onPress={() => dispatch(removeFromBasket({_id: key}))}>
                            Remove
                        </Text>
                    </TouchableOpacity>
                </View> 
        ...
        <TouchableOpacity onPress={() => (navigation as any).navigate('PreparingOrder')} className='rounded-lg bg-[#00CCBB] p-4'>
